...
/** Panel login/registrasi (email+password & Google). Dipakai di gerbang & halaman akun. */
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (tab === "login") {
        ...
        if (needsConfirm) {
          setOk("Registrasi berhasil! Cek email untuk konfirmasi, lalu masuk.");
          setTab("login");
        }
      }
    } catch (e) {
    ...
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-neutral-900">
      {!configured && (
        <div className="mb-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-800 ring-1 ring-amber-200">
          ...
          Isi <code className="rounded bg-amber-100 px-1">NEXT_PUBLIC_SUPABASE_ANON_KEY</code>. Kamu tetap
          bisa lanjut sebagai tamu.
        </div>
      )}
      ...
      {onGuest && (
        <button
          onClick={onGuest}
          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 py-2 text-sm font-medium text-foreground/55 hover:text-foreground"
        >
          Lanjut sebagai tamu <ArrowRight size={15} />
        </button>
      )}
    </div>
  );
